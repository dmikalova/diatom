...
- **After 3 failed gate attempts** (configurable), the task is retried once on a stronger profile (ADR 0006). If it still fails, it is parked as a question to the human with the failure output. A commit is never made while the gate is failing, and the gate is never skipped.

...
- **Revisions land as fixup commits**, one per original commit revised (`fixup! <original subject>`, or `fixup! <SHA>` when another commit of the goal has the same subject and autosquash could pick the wrong one). The agent marks each revision done as soon as it finishes it, and the harness splits the session's work into fixups at those points. The gate runs once, on the session's final files: a fixup's own tree is never merged anywhere, and autosquash moves it beside its original anyway. Once the goal is done, the human is offered `git rebase --autosquash` and a split into stacked pull requests, or a direct push. Commits are sized to keep the gate passing, not to one change each.
